...
extension PlaySoundsViewController: AVAudioPlayerDelegate {
    ...
    func setupAudio() {
        ...
        do {
            ...
            audio = try AVAudioPlayer(contentsOfURL: recordedAudioURL)
        } catch {
    ...
    func playSound(rate rate: Float? = nil, pitch: Float? = nil, echo: Bool = false, reverb: Bool = false) {
        ...
        //stop the current running audioEngine
        ...
        self.timer = NSTimer.scheduledTimerWithTimeInterval(1, target: self, selector: (#selector(PlaySoundsViewController.updateRemainingTime)), userInfo: nil, repeats: true)
    ...
    func updateRemainingTime() {
        ...
        if remainingT != nil {
            if remainingT < 1.0 {
                remainingTime.text = stringFromTimeInterval(remainingT)
            } else {
                remainingT = remainingT - Double(1.0)
                remainingTime.text = stringFromTimeInterval(remainingT)
            }
        } else {
            remainingT = audio.duration - 1
